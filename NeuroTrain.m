...
i=1;
N=hiddenLayerSize;
R=mean(R); %R - многомерный пар-р, число измерений соотв. числу пар-ров.
while (~exit)
   hiddenLayerSize=hiddenLayerSize+inc;
   N=[N hiddenLayerSize]; %Записываем в массив текущее кол-во нейр.
   i=i+1;
   net = fitnet(hiddenLayerSize,trainFcn);
   [net,tr] = train(net,x,t);
   ...
   disp(['Количество нейронов ' num2str(hiddenLayerSize) ' Показатель регрессии ' num2str(R_c) ' Итерации ' num2str(i)]);

   
   if (R_c>=R_nes)&&(inc==1) exit=true; end %Если достигли нужного значения - выходим.
   if (R_c<=R(i-1))||(hiddenLayerSize==1) %Чтобы не допустить обращение числа нейр. в 0
       %Иногда 1 нейрон показывает рез-т не хуже, чем 10.
       if inc==1 exit=true; end %Если inc=-1, то мы пробовали уменьшать, если inc=1, значит нашли максимальное кол-во нейр.
       %Если пробовали уменьшать, попробуем увеличивать. Если увеличивали и
       %достигли максимума - выходим.
       if (R_c<R(i-1))||(hiddenLayerSize==1)
           %В случае, когда уменьшаем число нейронов, ищем минимально
           %возможное их количество. Когда увеличиваем, останавливаемся, когда рез-т перестаёт улучшаться.
           inc=1;
           hiddenLayerSize=N(1); %Устанавливаем исходное кол-во нейр.
       end
   end
end
[mv,mi]=max(R); %Ищем реализацию с лучшим показателем
hiddenLayerSize=N(mi); %Уст. число нейронов, mi - индекс макс. показателя.
net = fitnet(hiddenLayerSize,trainFcn);
[net,tr] = train(net,x,t);
figure
plotregression(t,y)